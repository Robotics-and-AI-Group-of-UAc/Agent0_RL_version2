...
### Instalar a biblioteca Pillow:
  Após instalar o Python, executar na linha de comandos:  
    ...
    ```python3 -/home/jmc/MEGAsync/PROJECT-Agent0/Agent0_minotauro_RL_version2m pip install --upgrade Pillow```
...
## RL
A versão RL apresentada permite as seguintes funcionalidades que versões não RL não apresentavam.
A primeira é permitir a introdução de setas que correspondem às políticas do agente.
 A segunda é permitir a definição de "targets" que permitem ao agente movimentar-se no mundo e terminar a sua execução nessas casas.
