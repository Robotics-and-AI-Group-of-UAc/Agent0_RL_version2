...
from interface import game_board as gb
from interface import game_objects as go
...
class Server(BaseException):
    def __init__(self, host, port, config):
        ...
        self.port = port
        self.config = config
        self.sock = ZMQSocket(host,port,REP)
    ...
    def loop(self):
        ...
        while True:
            ...
            if len(data) >= 2:
                ...
                self.sock.send_msg(return_data)
                self.root.update()
                if return_data =="exit":
